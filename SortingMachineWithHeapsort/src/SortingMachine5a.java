...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        //if array is large enough not to be a heap then continue
        if (array.length > 1) {

            //assign indices to left and right subtree roots
            int left = 2 * top + 1;
            int right = left + 1;

            //if index of right is within the array, right is less than left,
            //and is less than top, exchange top and right and sift down
            if (right <= last && order.compare(array[left], array[right]) >= 0
                    && order.compare(array[right], array[top]) < 0) {
                exchangeEntries(array, top, right);
                siftDown(array, right, last, order);

                //if index of left is within the array and left is less than top,
                //switch top and left and sift down
            } else if (left <= last
                    && order.compare(array[left], array[top]) < 0) {
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        //assign left and right subtree root indices
        int left = 2 * top + 1;
        int right = left + 1;

        //if array is larger than size 0 continue
        if (array.length > 0) {

            //if the left subtree isn't in heap form heapify it
            if (!isHeap(array, left, array.length - 1, order)) {
                heapify(array, left, order);
            }

            //if the right subtree isn't in heap form heapify it
    ...
    public final int size() {
        ...
        //depending on the mode return size
        int size = 0;
        if (this.insertionMode) {
