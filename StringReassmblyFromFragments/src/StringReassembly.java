...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combo = str1.substring(0, str1.length() - overlap);
        combo = combo + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //check if str is a substring of any string in strSet
        while (strSet.size() > 0) {
            String tempString = strSet.removeAny();

            //if str is a substring of tempString contains = true
            if (tempString.contains(str)) {
                contains = true;
            }

            //add each string to tempSet
            tempSet.add(tempString);
        }

        //add strings back if they aren't a substring of str
        if (!contains) {

            //check if each string is a substring of str
            for (String tempString : tempSet) {
                if (!str.contains(tempString)) {
                    strSet.add(tempString);
                }
            }

            //add back to regular set if isn't a substring of str
            strSet.add(str);
        } else {
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //add all Strings without substrings
        while (!input.atEOS()) {
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //check each character isn't "~" if it is add a new line
