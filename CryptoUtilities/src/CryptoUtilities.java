...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        //if the remainder of n/2 is 0 n is even
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber modHolder = new NaturalNumber2(0);
        final NaturalNumber one = new NaturalNumber2(1);

        //check if p is zero, if it is n is 1
        if (!p.isZero()) {

            //check if p is one if it is n = n % m
            if (p.compareTo(one) > 0) {
                NaturalNumber pHalf = new NaturalNumber2(p);
                pHalf.divide(new NaturalNumber2(2));
                NaturalNumber temp = new NaturalNumber2(n);

                //check pHalf isn't less than or equal to 1 before recursing
                if (pHalf.compareTo(one) > 0) {
                    powerMod(n, pHalf, m);
                    NaturalNumber temp2 = new NaturalNumber2(n);
                    n.multiply(temp2);
                    modHolder.copyFrom(n.divide(m));
                    n.copyFrom(modHolder);
                } else {
                    n.multiply(temp);
                    modHolder.copyFrom(n.divide(m));
                    n.copyFrom(modHolder);
                ...
                //if p is odd multiply one extra time
                if (!isEven(p)) {
                    n.multiply(temp);
                    modHolder.copyFrom(n.divide(m));
                    n.copyFrom(modHolder);
     ...
     * @return true if w is a "witness" that n is composite
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber wHolder = new NaturalNumber2(w);
        NaturalNumber nHolder = new NaturalNumber2(n);

        powerMod(wHolder, new NaturalNumber2(2), n);

        if (wHolder.equals(new NaturalNumber2(1))) {
            isWitness = true;
        }

        wHolder.copyFrom(w);
        nHolder.decrement();
        powerMod(wHolder, nHolder, n);

        if (!isWitness && !wHolder.equals(new NaturalNumber2(1))) {
            isWitness = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            while (i < fifty && !isPrime) {
                randomNumber(checker);
                if (checker.compareTo(new NaturalNumber2(1)) > 0) {
                    isPrime = !isWitnessToCompositeness(checker, n);
                }
                i++;
            }
        }
